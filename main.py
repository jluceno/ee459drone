...
try:
  ...
  while True:
    ...
    imu_pitch = (accel_gain * (imu_pitch_last +(dt*gyro_y))) + ((1-accel_gain) * imu_pitch)
    imu_roll = (accel_gain * (imu_roll_last + (dt*gyro_x))) + ((1-accel_gain) * imu_roll)
    ...
    print("Duty cycles: ", "FL:", round((duty_cycle1 - 0x7fff)/0xffff * 100), "FR:", round((duty_cycle2 - 0x7fff)/0xffff * 100),
      "BR:", round((duty_cycle3 - 0x7fff)/0xffff * 100), "BL:", round((duty_cycle4 - 0x7fff)/0xffff * 100))
